...
class BaseReporter {
  ...
  generateStartingMessage() {
    ...
    const shardDetails = this.config.shard
      ...
      : "";
    return `\nRunning ${this.totalTestCount} test${
      this.totalTestCount > 1 ? "s" : ""
    } using ${jobs} worker${jobs > 1 ? "s" : ""}${shardDetails}`;
  }
  ...
  generateSummaryMessage({
    ...
    if (unexpected.length) {
      ...
      for (const test of unexpected)
        tokens.push(
          _safe.default.red(formatTestHeader(this.config, test, "    "))
        );
    }
    ...
    if (flaky.length) {
      ...
      for (const test of flaky)
        tokens.push(
          _safe.default.yellow(formatTestHeader(this.config, test, "    "))
        );
    }
    ...
    if (expected)
      tokens.push(
        _safe.default.green(`  ${expected} passed`) +
          _safe.default.dim(` (${(0, _ms.default)(this.duration)})`)
      );
    ...
    if (this.suite.allTests().length) {
      const failureRate = (
        unexpected.length /
        (expected + unexpected.length + flaky.length) *100
      ).toPrecision(2);
      tokens.push(
        `${_safe.default.dim("  Failure Rate:")} ${
          failureRate < 5
            ? _safe.default.green(failureRate + "%")
            : _safe.default.red(failureRate + "%")
        } ${_safe.default.dim("of finished tests")}`
      );
      // tokens.push(_safe.default.dim(`  Failure Rate: ${failureRate >=5 ? _safe.default.red(failureRate) : _safe.default.green(failureRate)}%`))
      tokens.push(
        _safe.default.dim(`  Average test duration: ${aveDuration}s`)
      );
    }
    return tokens.join("\n");
  }
  ...
  generateSummary() {
    ...
    this.suite.allTests().forEach((test) => {
      ...
      totalDuration += test.results.length ? test.results[0].duration : 0;
    });
    const failuresToPrint = [...unexpected, ...flaky, ...skippedWithError];
    const aveDuration = (
      totalDuration /
      this.suite.allTests().length /
      1000
    ).toFixed(1);
    return {
      skipped,
  ...
  _printSlowTests() {
    ...
    slowTests.forEach(([file, duration]) => {
      console.log(
        _safe.default.yellow("  Slow test file: ") +
          file +
          _safe.default.yellow(` (${(0, _ms.default)(duration)})`)
      );
    });
    if (slowTests.length)
      console.log(
        _safe.default.yellow(
...
function formatFailure(config, test, options = {}) {
  ...
  for (const result of test.results) {
    const resultLines = [];
    const errors = formatResultFailure(
      config,
      test,
      result,
      "    ",
      _safe.default.enabled
    );
    ...
    if (result.retry) {
      retryLines.push("");
      retryLines.push(
        _safe.default.gray(pad(`    Retry #${result.retry}`, "-"))
      );
    }
      ...
      for (let i = 0; i < result.attachments.length; ++i) {
        ...
        if (attachment.path) {
          ...
          if (attachment.name === "trace") {
            ...
            resultLines.push(
              _safe.default.cyan(
                `        npx playwright show-trace ${relativePath}`
              )
            );
            resultLines.push("");
          }
    ...
    if (includeStdio && output.length) {
      const outputText = output
        .map(({ chunk, type }) => {
          ...
          if (type === "stderr")
            return _safe.default.red(stripAnsiEscapes(text));
          return text;
        })
        .join("");
      resultLines.push("");
      resultLines.push(
        _safe.default.gray(pad("--- Test output", "-")) +
          "\n\n" +
          outputText +
          "\n" +
          pad("", "-")
...
function formatResultFailure(
  ...
  if (result.status === "passed" && test.expectedStatus === "failed") {
    errorDetails.push({
      message: indent(
        _safe.default.red(`Expected to fail, but passed.`),
        initialIndent
      ),
    });
  }
...
function relativeFilePath(config, file) {
  return (
    _path.default.relative(config.rootDir, file) || _path.default.basename(file)
  );
}
