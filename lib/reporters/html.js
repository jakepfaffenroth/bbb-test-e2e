...
class HtmlReporter {
  ...
  _resolveOptions() {
    ...
    if (outputFolder)
      outputFolder = _path.default.resolve(
    ...
    return {
      outputFolder:
        (_ref =
          (_reportFolderFromEnv = reportFolderFromEnv()) !== null &&
          _reportFolderFromEnv !== void 0
            ? _reportFolderFromEnv
            : outputFolder) !== null && _ref !== void 0
          ? _ref
          : defaultReportFolder(this.config._configDir),
      open:
  ...
  async onEnd() {
    ...
    await (0, _fileUtils.removeFolders)([outputFolder]);
    const builder = new HtmlBuilder(outputFolder);
    const { ok, singleTestId } = await builder.build(
      new _raw.default().generateAttachments(this.config),
      reports
    );
    ...
    if (shouldOpen) {
      ...
      const relativeReportPath =
        outputFolder === standaloneDefaultFolder()
          ? ""
          : " " + _path.default.relative(process.cwd(), outputFolder);
      console.log("");
...
function defaultReportFolder(searchForPackageJson) {
  ...
  if (basePath) basePath = _path.default.dirname(basePath);
...
function standaloneDefaultFolder() {
  ...
  return (_reportFolderFromEnv2 = reportFolderFromEnv()) !== null &&
    _reportFolderFromEnv2 !== void 0
    ? _reportFolderFromEnv2
    : defaultReportFolder(process.cwd());
}

async function showHTMLReport(reportFolder, testId) {
  const folder =
    reportFolder !== null && reportFolder !== void 0
      ? reportFolder
      : standaloneDefaultFolder();
  ...
  console.log("");
  console.log(
    _safe.default.cyan(`  Serving HTML report at ${url}. Press Ctrl+C to quit.`)
  );
...
class HtmlBuilder {
  ...
  async build(testReportAttachments, rawReports) {
    const data = new Map();
    ...
    for (const [fileId, { testFile, testFileSummary }] of data) {
      ...
      const testCaseSummaryComparator = (t1, t2) => {
        const w1 =
          (t1.outcome === "unexpected" ? 1000 : 0) +
          (t1.outcome === "flaky" ? 1 : 0);
        const w2 =
          (t2.outcome === "unexpected" ? 1000 : 0) +
    ...
    const appFolder = _path.default.join(
    ...
    _fs.default.copyFileSync(
      _path.default.join(appFolder, "index.html"),
      _path.default.join(this._reportFolder, "index.html")
    ); // Copy trace viewer.

    if (this._hasTraces) {
      const traceViewerFolder = _path.default.join(
        require.resolve("playwright-core"),
        "..",
        "lib",
        "webpack",
      ...
      const traceViewerTargetFolder = _path.default.join(
        this._reportFolder,
        "trace"
      );
      ...
      for (const file of _fs.default.readdirSync(traceViewerFolder)) {
        ...
        _fs.default.copyFileSync(
          _path.default.join(traceViewerFolder, file),
          _path.default.join(traceViewerTargetFolder, file)
        );
      }
    } // Inline report data.
    ...
    await new Promise((f) => {
      this._dataZipFile.end(undefined, () => {
        this._dataZipFile.outputStream
          ...
          .pipe(
            _fs.default.createWriteStream(indexFile, {
              flags: "a",
            })
          )
          .on("close", f);
  ...
  _processJsonSuite(suite, fileId, projectName, path, outTests) {
    ...
    suite.suites.map((s) =>
      this._processJsonSuite(s, fileId, projectName, newPath, outTests)
    );
    suite.tests.forEach((t) =>
      outTests.push(this._createTestEntry(t, projectName, newPath))
    );
  }
      ...
      .map((a) => {
        ...
        if (a.path) {
          ...
          try {
            ...
            const sha1 =
              (0, _utils.calculateSha1)(buffer) + _path.default.extname(a.path);

            fileName = "data/" + sha1;

            _fs.default.mkdirSync(
              _path.default.join(this._reportFolder, "data"),
              {
                recursive: true,
              }
            );
            ...
            _fs.default.writeFileSync(
              _path.default.join(this._reportFolder, "data", sha1),
              buffer
            );
          } catch (e) {
            return {
        ...
        if (a.body instanceof Buffer) {
          if (isTextContentType(a.contentType)) {
            ...
            const charset =
              (_a$contentType$match = a.contentType.match(/charset=(.*)/)) ===
                null || _a$contentType$match === void 0
                ? void 0
                : _a$contentType$match[1];
